...
async function initializePermissions(redisService) {
    try {
        ...
        // Fetch or create permissions in the database using Sequelize models
        const permissionModel = models.permissions;
        const [dbPermissions, created] = await permissionModel.findOrCreate({
            where: {},
            defaults: permissions,
            individualHooks: true
        });

        const permissionList = created ? permissions : dbPermissions;

        // Cache permissions in Redis if available
        if (redisService && redisService.enabled) {
            await redisService.set(cacheKey, JSON.stringify(permissionList), 3600); // Cache for 1 hour
