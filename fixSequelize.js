/**
 * إصلاح مشكلة Sequelize مع عمود role في جدول role_permissions
 */
const db = require('./models').default;
const sequelize = require('./config/sequelize');

async function fixSequelizeIssue() {
    try {
        // 1. طباعة معلومات النموذج للتشخيص
        console.log('فحص المشكلة...');
        console.log('تعريف نموذج role_permissions:', 
            Object.keys(db.role_permissions.rawAttributes)
        );

        // 2. تسجيل الخروج والانتهاء
        console.log('تعريف النماذج مكتمل.');
        
        // 3. إغلاق اتصالات Sequelize
        await sequelize.close();
        console.log('تم إغلاق اتصالات Sequelize بنجاح.');
        
        console.log('\n');
        console.log('للتصحيح النهائي، يرجى اتباع الخطوات التالية:');
        console.log('1. أوقف خادم التطبيق تمامًا');
        console.log('2. نفذ الأمر التالي في سطر الأوامر:');
        console.log('   cd d:\\testProjecte\\TravelerBloggers-main (1)\\TravelerBloggers && node patchModels.js');
        console.log('3. أعد تشغيل خادم التطبيق');
...
fs.writeFileSync(path.join(__dirname, 'patchModels.js'), patchCode);
console.log('تم إنشاء ملف patchModels.js');
